...
def verify_conditions(model, local_bounds=(np.pi ** 2, 4), epsilon=1e-2, num_samples=32*20):
    """
...
def train_and_save_model(model, dataset, max_iterations, batch_size, n_epochs):
    ...
    print(f"Saving trained model to model/model_storage_pendulum_no_shaping.pth...")
    # # Save the model
    torch.save(model.state_dict(), "model/model_storage_pendulum_no_shaping.pth")
...
if __name__ == "__main__":
    ...
    model_storage_test.load_state_dict(torch.load("model/model_storage_pendulum_no_shaping.pth"))
