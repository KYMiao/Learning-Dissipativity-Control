...
def train_and_save_model(model, dataset, max_iterations, batch_size, n_epochs):
    ...
    print(f"Saving trained model to model/model_storage_circuit.pth...")
    # # Save the model
    torch.save(model.state_dict(), "model/model_storage_circuit.pth")
...
class Storage(pl.LightningModule):
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.log('c', loss5, prog_bar=True, logger=True)
        return  10 * loss1 + 10 * loss2 + 10 * loss3 + 10 * loss4 + 0.001 * loss5
...
def test_and_visualize(model_storage, n_samples=1, num_states=100):
    ...
    for sample_index in range(n_samples):
        ...
        for state in selected_trajectory:
            ...
            Ve, _ = compute_clf(model_storage, xe_tensor, compute_control(model_storage, xe_tensor))
            Ve = Ve - xe_tensor @ torch.tensor([[1.1338, -0.1547], [-0.1547, 1.4574]], dtype=torch.float32) @ xe_tensor.transpose(-1, -2)
            print(f"Storage-LQR {[Ve.item()]}")
...
def plot_loss_distribution(model_storage):
    ...
    for _ in range(n_batches):
        ...
        batch_states = torch.stack([
            torch.tensor([
                torch.empty(1).uniform_(-0.20614524232402465, -0.10614524232402466),  # Pendulum angle
                torch.empty(1).uniform_(0.10625, 0.20624999999999999),  # Angular velocity
            ])
    ...
    plt.figure(figsize=(10, 6))
    plt.hist(loss_values3.numpy(), bins=30, edgecolor='black', alpha=0.7)
    plt.title("Distribution of fc3 on Training Data")
    plt.xlabel("Loss Value (fc3)")
    plt.ylabel("Frequency")
    plt.grid(axis='y', linestyle='--', alpha=0.7)
    plt.show()
...
if __name__ == "__main__":
    ...
    model_storage_test.load_state_dict(torch.load("model/model_storage_circuit_no_shaping.pth"))
