...
def verify_conditions(model, local_bounds=(1, 1, 1), epsilon=1e-2, num_samples=32*20):
    """
...
def train_and_save_model(model, dataset, max_iterations, batch_size, n_epochs):
    ...
    print(f"Saving trained model to model/model_storage_pendulum_dynamic_clf.pth...")
    # # Save the model
    torch.save(model.state_dict(), "model/model_storage_pendulum_dynamic_clf.pth")
...
def test_and_visualize(model_storage, n_samples=1, num_states=100):
    ...
    for sample_index in range(n_samples):
        ...
        initial_state = torch.tensor([
            torch.empty(1).uniform_(-0.2 * np.pi, 0.2 * np.pi),  # Initial angle theta
            torch.empty(1).uniform_(-0.2, 0.2),           # Initial angular velocity theta_dot
            torch.empty(1).uniform_(0, 0),
        ]).float()
        ...
        for i, state in enumerate(selected_trajectory):
            ...
            print(f"State {i + 1}: Pendulum angle, velocity, xc {[theta, theta_dot, state[2]]}")

            # Print the control input for the current state
            control_input = control_input.cpu().detach().numpy()
            print(f"State {i + 1}: Control = {[control_input[0, 1]]} ")
...
if __name__ == "__main__":
    ...
    model_storage_test.load_state_dict(torch.load("model/model_storage_pendulum_dynamic_clf.pth"))
